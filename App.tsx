...
const holePoints = (player: Player, hole: Hole) =>
  scoreEntered(player.scores[hole.number])
    ...
    : 0;
const totalPoints = (player: Player) => course.reduce((sum, hole) => sum + holePoints(player, hole), 0);
const completed = (player: Player) => course.filter((hole) => scoreEntered(player.scores[hole.number])).length;
...
function starterRound(): RoundState {
  ...
  return normalizeRound({
    ...
    groups,
    players: [
      { id: id("p"), name: "Chris", handicap: 11, groupId: a.id, scores: { 1: "5", 2: "4", 3: "3", 4: "6" } },
      { id: id("p"), name: "Martin", handicap: 17, groupId: a.id, scores: { 1: "5", 2: "6", 3: "4", 4: "7" } },
      { id: id("p"), name: "Neil", handicap: 22, groupId: a.id, scores: { 1: "6", 2: "5", 3: "4", 4: "6" } },
      { id: id("p"), name: "Adrian", handicap: 9, groupId: a.id, scores: { 1: "4", 2: "4", 3: "3", 4: "5" } },
      { id: id("p"), name: "Steve", handicap: 14, groupId: b.id, scores: { 1: "5", 2: "5", 3: "3", 4: "6" } },
      { id: id("p"), name: "Paul", handicap: 19, groupId: b.id, scores: { 1: "6", 2: "5", 3: "4", 4: "6" } },
      { id: id("p"), name: "Graham", handicap: 24, groupId: b.id, scores: { 1: "6", 2: "6", 3: "4", 4: "7" } },
    ],
    ghosts: {},
  });
}
...
export default function App() {
      ...
      try {
        ...
        if (info.exists) {
          ...
          if (!cancelled && parsed.savedRounds) {
            setSavedRounds(parsed.savedRounds.map((item) => ({ ...cloneRound(item), savedAt: item.savedAt })));
          }
        }
        if (!cancelled) {
  ...
  const currentHole = course.find((hole) => hole.number === selectedHole) ?? course[0];
  const rankedPlayers = useMemo(
    () =>
      [...round.players]
        .map((player) => ({ ...player, total: totalPoints(player), done: completed(player) }))
    ...
    () =>
      round.groups
        .map((group) => {
          ...
          const holeTotal =
            members.reduce((sum, player) => sum + holePoints(player, currentHole), 0)
            + (members.length === 3 && ghost ? holePoints(ghost, currentHole) : 0);
          return { ...group, members, ghost, size: members.length, total, holeTotal };
        })
        .sort((a, b) => b.total - a.total),
    [currentHole, playerMap, rankedPlayers, round.ghosts, round.groups],
  );
          ...
          <>
                  ...
                  <View key={player.id} style={styles.rankRow}>
                    <View style={styles.rankLeft}>
                      ...
                      <View>
                        ...
                        <Text style={styles.meta}>
                          {round.groups.find((group) => group.id === player.groupId)?.name ?? "Group"} • HI {player.handicap} • {player.done}/18 holes
                        </Text>
                      </View>
                    </View>
                    <View style={styles.badge}>
            ...
            <View style={styles.card}>
              <View style={styles.subCard}>
                ...
                <Text style={styles.meta}>Par {currentHole.par} • Stroke Index {currentHole.strokeIndex}</Text>
              </View>
              {round.groups.map((group) => {
                    ...
                    {members.map((player) => (
                      <View key={player.id} style={styles.scoreRow}>
                        <View style={styles.scoreInfo}>
                          <Text style={styles.itemText}>{player.name}</Text>
                          <Text style={styles.meta}>HI {player.handicap} • Gets {strokes(player.handicap, currentHole.strokeIndex)} shot{strokes(player.handicap, currentHole.strokeIndex) === 1 ? "" : "s"}</Text>
                        </View>
                        <TextInput
                        ...
                        <View style={styles.pointsBox}>
                          <Text style={styles.pointsText}>{holePoints(player, currentHole)}</Text>
                          <Text style={styles.badgeLabel}>pts</Text>
                        </View>
                      </View>
                    ))}
          ...
          <>
            ...
            {savedRounds.length === 0 ? (
              ...
              savedRounds.map((saved) => {
                const players = [...saved.players].map((player) => ({ ...player, total: totalPoints(player), done: completed(player) })).sort((a, b) => b.total - a.total);
